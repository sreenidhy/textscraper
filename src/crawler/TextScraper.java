...
public class TextScraper {
	...
	String totalResult;		// total number of result for given search string
	String link;			// part of url that does the searching
	String searchString;	// item to be searched for
	String pageNumber;		// specific page number from all the pages returned for this search
	Result result;			// hold all the individual products
	...
	private void populateItem(Item it, Element prodInfo) {
		...
		String itemName, itemPrice, checkoutPrice = null;
		...
		if (null != prodInfo) {
			...
			if (bigPriceText.text() == "" || bigPriceText.text().length() == 0) {
				// price is mentioned as out of stock
			...
			if ((bigPriceText.text() == "" || bigPriceText.text().length() == 0) &&
					(smallPriceText.text() == "" || smallPriceText.text().length() == 0)) {
				// price is available at check out
				checkoutPrice = prodInfo.getElementsByAttributeValue("class", "PriceLBold").text();
			...
			itemPrice = (null == checkoutPrice || checkoutPrice.length() == 0) ? 
					(bigPriceText.text() + smallPriceText.text()) : 
					(checkoutPrice);
			if (DEBUG) {
